...
class PygameAdapter:
    def __init__(self, app_name, source, action_handler):
        if PLATFORM_SYSTEM == "Darwin":
            ...
            self.adapter = accesskit.unix.Adapter.create(app_name, "pygame", pygame.version.ver, source, action_handler)
...
def do_action(request):
    if request.action in [accesskit.Action.Default, accesskit.Action.Focus]:
        args = {
            "event": SET_FOCUS_MSG if request.action == accesskit.Action.Focus else DO_DEFAULT_ACTION_MSG,
